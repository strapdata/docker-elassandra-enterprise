...
CREDENTIAL="--user cassandra:cassandra"
PROTOCOL=https
NODE=cassandra
curl() {
	docker run -i --rm \
...
get() {
	docker run -i --rm \
		--link "$cid":cassandra \
		"$clientImage" \
    curl -XGET  $CREDENTIAL  "$PROTOCOL://$NODE:9200/$1" $2 $2 $4 $5 --fail -k
}
put() {
	docker run -i --rm \
		--link "$cid":cassandra \
		"$clientImage" \
    curl -XPUT -H Content-Type:application/json $CREDENTIAL "$PROTOCOL://$NODE:9200/$1" -d "$2" --fail -k
}
post() {
    docker run -i --rm \
		--link "$cid":cassandra \
		"$clientImage" \
    curl -XPOST -H Content-Type:application/json $CREDENTIAL "$PROTOCOL://$NODE:9200/$1" -d "$2" --fail -k
}

delete() {
   curl -XDELETE -H Content-Type:application/json $CREDENTIAL "$PROTOCOL://$NODE:9200/$1" -d "$2" --fail -k
}
